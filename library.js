    ...
    passport.use(new passportLocal(function(username, password, next) {
      ...
      async.waterfall([
        ...
        function(res, next) {
          ...
          user.getUidByUserslug(userAccount.profile.slug, next);
        },
        function(_uid, next) {
          if (!_uid) { // register a user in local database
            return user.create({username: userAccount.profile.slug, email: userAccount.emails.length > 0 ? userAccount.emails[0].address : ""}, next);
          }
          next(null, _uid);
        },
        function(_uid, next) {
